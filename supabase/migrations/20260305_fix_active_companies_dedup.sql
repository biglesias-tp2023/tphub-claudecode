-- ============================================
-- Migration: 20260305_fix_active_companies_dedup.sql
-- Description: Fix DISTINCT ON bug in all 5 daily alert RPCs.
--   The active_companies CTE was filtering by status BEFORE
--   deduplication, so deactivated companies (e.g. Supernormal,
--   Gracias Padre) could appear if an older snapshot had an
--   active status. Fix: deduplicate first, then filter.
--
-- Date: 2026-03-05
-- ============================================
  ...
  active_companies AS (
    SELECT * FROM (
      SELECT DISTINCT ON (pk_id_company)
        pk_id_company, des_company_name, des_status, flg_deleted, des_key_account_manager
